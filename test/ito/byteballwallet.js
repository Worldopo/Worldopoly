...
export default function (Token, Crowdsale, wallets) {
  ...
  it('should mint 5000000 tokens to byteballwallet after ito finish', async function () {
    const owner = await crowdsale.owner();
    await crowdsale.finish({from: owner});
    const bbwallet = await crowdsale.bbwallet();
    const balance = await token.balanceOf(bbwallet);
    balance.should.bignumber.equal(tokens(5000000));
  });
